...
class TestMasterDefendantId(IntegrationTestBase):
    ...
    async def test_unaffected_when_both_null(
        self,
        ...
        weight = await self._score_weight_between(person_1, person_2, pg_db_url, db_connection)
        assert -40 < weight < 40
    ...
    async def test_weight_changes_only_for_equal_master_defendant_id(
        ...
        assert 19.0 <= equal_weight - baseline_weight <= 21.0
        ...
        assert abs(mismatch_weight - baseline_weight) < 1.0
        ...
        assert abs(one_null_weight - baseline_weight) < 1.0
