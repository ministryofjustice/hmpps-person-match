...
class Person(BaseModel):
    ...
    date_of_birth: datetime.date | None = Field(
        alias="dateOfBirth", format="date", default=None, examples=["1970-01-01"],
    )  # Ensures YYYY-MM-DD
    ...
    date_of_birth_aliases: list[datetime.date] = Field(
        alias="dateOfBirthAliases", format="date", examples=[["1970-01-01"]],
    )  # Ensures YYYY-MM-DD
    ...
    sentence_dates: list[datetime.date] = Field(
        alias="sentenceDates", format="date", examples=[["2025-01-01"]],
    )  # Ensures YYYY-MM-DD
