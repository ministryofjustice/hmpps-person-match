...
async def get_person_search_by_match_id(
    match_id: str,
    session: TransactionalSession,
    logger: Annotated[Logger, Depends(get_logger)],
) -> list[PersonScore]:
    """
    Person search GET request handler
    Returns a list of scored candidates against the provided record match identifier.

    Uses a looser set of blocking rules than /person/score/{match_id}, so as
    to return a larger set of scored candidates.
    """
    if await score.match_record_exists(match_id, session):
        ...
        logger.info(
            ...
            extra=dict(matchId=match_id, candidate_size=len(scored_candidates)),
        )
