...
class TestPersonSearchRoute:
    """
    Test Person Search by match_id (GET) Route
    """
    ...
    def test_person_search_no_results(
        ...
        response = call_endpoint("get", self._generate_match_search_url(match_id), roles=[Roles.ROLE_PERSON_MATCH])
        assert response.status_code == 200
        assert response.json() == []
        mock_logger.info.assert_called_with(
            TelemetryEvents.PERSON_SEARCH_BY_MATCH_ID,
            extra={
                "matchId": match_id,
                "candidate_size": 0,
            },
    ...
    def test_person_search_with_results(
        ...
        response = call_endpoint(
            "get",
            self._generate_match_search_url(searching_person),
    ...
    def test_missing_record_returns_404(self, call_endpoint: Callable, mock_existence_check: AsyncMock) -> None:
        ...
        response = call_endpoint(
            "get",
            self._generate_match_search_url(),
            roles=[Roles.ROLE_PERSON_MATCH],
        )
    ...
    def test_invalid_role_unauthorized(self, call_endpoint: Callable) -> None:
        ...
        """
        response = call_endpoint("get", self._generate_match_search_url(), roles=["Invalid Role"], json={})
    ...
    def test_no_auth_returns_unauthorized(self, client: TestClient) -> None:
        response = client.get(self._generate_match_search_url())
        assert response.status_code == 401
        assert response.json()["detail"] == "Not authenticated"

    @staticmethod
    def _generate_match_search_url(match_id: str = str(uuid.uuid4())) -> str:
